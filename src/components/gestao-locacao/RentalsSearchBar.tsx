...
interface RentalsSearchBarProps {
  placeholder?: string;
  value?: string;
...
export const RentalsSearchBar = ({
  placeholder = 'Buscar na Gestão de Aluguéis por contrato, locador, locatário ou imóvel',
  value,
  onChange,
  filtersCount = 0,
  className,
  filtersTitle = 'Filtros de Gestão de Aluguéis',
  filtersDescription = 'Refine sua busca usando os filtros abaixo',
  filtersContent,
  hasActiveFilters = false,
  onApplyFilters,
  onClearFilters
}: RentalsSearchBarProps) => {
  ...
  return (
    <div className={cn('flex items-center gap-3', className)}>
      <div className="relative flex-1">
        ...
        <input
          type="text"
          aria-label="Buscar na gestão de aluguéis"
      ...
      <Sheet open={isFilterOpen} onOpenChange={setIsFilterOpen}>
        <SheetTrigger asChild>
          ...
          >
            <Filter className={cn('w-5 h-5', filterIsActive ? 'text-[hsl(var(--accent))]' : 'text-[hsl(var(--icon))]')} />
            {filtersCount > 0 && (
