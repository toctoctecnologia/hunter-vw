  ...
  useEffect(() => {
    try {
      ...
      if (tokensParam) {
        ...
        saveGoogleTokens('calendar', tokens);

        // Notifica a janela pai (se aberto em popup)
        if (window.opener) {
          window.opener.postMessage({ type: 'GOOGLE_CALENDAR_SUCCESS', tokens }, window.location.origin);
        }
      }
    } catch (err) {
            ...
            <>
              ...
              <div className="space-y-2 text-center">
                ...
                <p className="text-muted-foreground">
                  Seu calendário foi sincronizado com sucesso. Agora você pode fechar esta janela.
                </p>
              </div>
            </>
          )}
