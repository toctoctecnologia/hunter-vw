...
export function FeaturesExtraCard({ form }: DataCardProps) {
  const [pagination] = useState({ pageIndex: 0, pageSize: 100 });

  const { data: propertyFeatures = [] } = useQuery({
    queryKey: ['property-features', pagination, PropertyFeatureType.EXTRA],
    queryFn: () => getPropertyFeatures(pagination, '', PropertyFeatureType.EXTRA),
  });
