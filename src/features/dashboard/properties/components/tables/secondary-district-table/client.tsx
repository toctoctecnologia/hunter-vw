...
import { columns } from '@/features/dashboard/properties/components/tables/secondary-district-table/columns';
import { getDistricts } from '@/features/dashboard/properties/api/secondary-district';
import { Loading } from '@/shared/components/loading';
import { useAuth } from '@/shared/hooks/use-auth';
import { hasFeature } from '@/shared/lib/permissions';

export function SecondaryDistrictClient() {
  const { user } = useAuth();
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['secondary-district', pagination],
    queryFn: () => getDistricts(pagination, search),
  });

  return (
    <>
      <SaveSecondaryDistrictModal open={isModalOpen} onClose={() => setIsModalOpen(false)} />
