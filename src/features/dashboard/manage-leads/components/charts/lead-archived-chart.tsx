...
import { getGradientColor, LeadOriginTypeToLabel } from '@/shared/lib/utils';

import { getArchivedByChannelMetrics } from '@/features/dashboard/manage-leads/api/dashboard';
...
export function LeadArchivedChart() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['archived-by-channel-metrics', startDate, endDate],
    queryFn: () => getArchivedByChannelMetrics(startDate, endDate),
  });
    ...
    <Card className="flex flex-col">
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle>Leads Arquivados por Canal</CardTitle>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                  ...
                  content={
                    <ChartTooltipContent
                      formatter={(value, name) => {
                        ...
                        return (
                          <div>
                            <span className="block font-medium">{LeadOriginTypeToLabel(metric.originType)}</span>
            ...
            <div className="flex flex-col gap-3">
              {data.map((item) => (
                <div key={item.originType} className="flex items-center gap-3">
                  <div
                    className="w-3 h-3 rounded-full shrink-0"
                    style={{ backgroundColor: getGradientColor(item.percentage) }}
                  />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate">{LeadOriginTypeToLabel(item.originType)}</p>
                  </div>
